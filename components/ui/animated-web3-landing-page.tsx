...
export function Web3HeroAnimated({ showNav = false }: { showNav?: boolean }) {
      ...
      */}
      <style suppressHydrationWarning>
        {`
          ...
          /* Ensure height transitions work (paired with Tailwind duration/easing) */
          .transition-height {
      ...
      <section className="relative isolate h-[92vh] overflow-hidden bg-black text-white">
        {/* ================== BACKGROUND ================== */}
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-5xl place-items-center px-6 py-16 md:py-24 lg:py-28">
          ...
          <div className={`mx-auto text-center ${isMounted ? 'animate-fadeInUp' : 'opacity-0'}`}>
            {/* Badges */}
            <div className="inline-flex flex-wrap items-center justify-center gap-x-3 gap-y-2 text-[12px] text-white/80">
              <span className="rounded-full bg-white/5 px-3 py-1 ring-1 ring-white/10 backdrop-blur">Small cohorts (6–8)</span>
              <span className="rounded-full bg-white/5 px-3 py-1 ring-1 ring-white/10 backdrop-blur">Live & interactive</span>
              <span className="rounded-full bg-white/5 px-3 py-1 ring-1 ring-white/10 backdrop-blur">Portfolio-first</span>
            </div>
            ...
            <p style={{ animationDelay: '300ms' }} className={`mx-auto mt-5 max-w-2xl text-balance text-white/80 md:text-lg ${isMounted ? 'animate-fadeInUp' : 'opacity-0'}`}>
              Train as a job-ready developer in a 16-week, story-driven bootcamp—AI/ML to GenAI—learning through real client projects and simulated “Chaos Sprints.”
            </p>
            {/* CTAs */}
            <div style={{ animationDelay: '400ms' }} className={`mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row ${isMounted ? 'animate-fadeInUp' : 'opacity-0'}`}>
              <a href="/admissions" className="inline-flex items-center justify-center rounded-full bg-white px-6 py-3 text-sm font-semibold text-black shadow transition hover:bg-white/90">Apply Now</a>
              <a href="/admissions#waitlist" className="inline-flex items-center justify-center rounded-full border border-white/20 px-6 py-3 text-sm font-semibold text-white/90 backdrop-blur hover:border-white/40">Join Waitlist</a>
            </div>
          </div>
        </div>
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-0 h-[54vh]">
          ...
          <div className="absolute inset-x-0 bottom-0 flex h-full items-end gap-px px-[2px]">
            {pillars.map((h, i) => (
              <div
                key={i}
                className="transition-height duration-1000 ease-in-out flex-1 bg-black"
                style={{
